...
/*设置可预约数量按钮*/
 input.setbutton{
    ...
    margin-left: 4px;
    border:none;
    background-color: #FC8A15 !important;
    color: #fff;
    cursor: pointer;
}
...
<!--						<th class="header">保洁类型</th>-->
				...
				<tbody id="info">
					<c:forEach items="${record}" var="record">
						<tr>
							<td id="cleanApplyId" style="display: none">${record["RECORD_ID"]}</td>
							<td>${record["BRANCHNAME"]}</td>
							<td>${record["CLEANTIME"]}</td>
							<td>${record["ROOM_ID"]}</td>
							<td>${record["CLEAN_PERSON"]}</td>
							<td>${record["DTIMEAREA"]}</td>
							<td>${record["RECORD_USER"]}</td>
							<td>${record["REMARK"]}</td>
							<td>${record["DSTATUS"]}</td>
