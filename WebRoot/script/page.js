var page_id, model_id, page_conditions, grid_name, chart_type, query_url, query_flag, editFlag; 
var load_flag, last_row, last_cell, required_data, key_config, checked_row, data_changed, id_name; 
		...
		$.each(conditions, function(index) {
			...
			if ($("#cond_" + condition).is("input") && (/(DATE|MONTH|YEAR|TIME)$/i.test(condition))) {
				...
				$("#cond_" + condition).datepicker({
					dateFormat: el, 
					changeMonth: /MONTH/i.test(condition), 
					changeYear: /YEAR$/i.test(condition) 
				});
				isFirstDay = $("#cond_" + condition).attr("defaultValue") && $("#cond_" + condition).attr("defaultValue") == "First";
				$("#cond_" + condition).datepicker("setDate", isFirstDay ? new Date((new Date()).setUTCDate(1)) : new Date());
			}
		});
...
function setGridModel() {
	...
	$.each(page_columns, function(index) {
		...
		var col = { name: column.columnId, index: column.columnId, text: column.name, width: column.width || 120, frozen: column.frozen,
				...
				edithidden: column.edithidden, threshold: column.threshold, thresholdParam: column.thresholdParam, cellattr: addCellAttr };
...
function editData(rowid) {
	...
	if (page_config.editFunc) {
		callCustomFunc(page_config.editFunc, rowid);
   		...
   		if (page_config.functions && page_config.functions.EDIT && (!page_config.editing || page_config.editing == "form")) {
...
function formEditing(rowid, editData) {
	...
	$(grid_name).jqGrid("editGridRow", formId, {
   		url: base_path + "/" + url + ".do?modelId=" + model_id + "&pageId=" + page_id 
   			+ "&dealType=" + (rowid ? update_tag : save_tag) + "&keyConfig=" + key_config,
   		resize: false,
		...
		savekey: [true, 13],
   		onInitializeForm: function(form, oper) {
   			...
   			if (custom_dialog.length > 0) {
   				var cm = $(grid_name).jqGrid("getGridParam", "colModel"), col;
				
   				$.each(custom_dialog, function(index) {
   					col = custom_dialog[index];
   					var customFunc = col["customFunc"], colName = col["name"], vl;
...
function concatEl(colName, value) {
...
function openSubWin(title, pageId, params, width, height, style) {
	var url = base_path + "/queryPageConfig.do?modelId=" + model_id + "&pageId=" + pageId 
		+ "&initParams=" + encodeURIComponent(JSON.stringify(params));
	width = width || screen.availWidth - 240;
	height = height || screen.availHeight - 240;
	var option = "height=" + height + ",width=" + width + ",scroll=0,status=0,location=0";
	
	if (!style || style == "dialog" || style == "dialog-radio") {
